...
class AreaCalculator:
    def __init__(self, name, hand_image:np.ndarray, area_of_paper:float) -> None:
        ...
        self.__hsv_threshold_value = [127.58992805755395, 255.0]
    ...
    def calculate_paper_area(self) -> float:
        ...
        cv2.drawContours(self.hand_image, [paper], 0, (0, 255, 0), 2)
        return paper, cv2.contourArea(paper)

    def calculate_hand_inside(self, paper_contour) -> float:
        """Returns area of page in PIXELS"""
        hsvimg = cv2.cvtColor(self.hand_image, cv2.COLOR_BGR2HSV)
        mask = cv2.inRange(hsvimg, (self.__hsv_threshold_hue[0], self.__hsv_threshold_saturation[0], self.__hsv_threshold_value[0]),  (self.__hsv_threshold_hue[1], self.__hsv_threshold_saturation[1], self.__hsv_threshold_value[1]))
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contours_sorted = sorted(contours, key=lambda x: cv2.contourArea(x), reverse=True)
        paper = contours_sorted[0]
        square = contours_sorted[1]
        return square, cv2.contourArea(square)
...
if __name__ == "__main__":
    # areacalculator = AreaCalculator("JUN",  cv2.imread("images/jundarkhand2.JPG"), Paper.LETTER)
    # areacalculator = AreaCalculator("GENE",  cv2.imread("images/genedarkhand.JPG"), Paper.LETTER)
    # areacalculator = AreaCalculator("TIRMIZI",  cv2.imread("images/tirmizihand.JPG"), Paper.LETTER)
    areacalculator = AreaCalculator("JUNboundingrect",  cv2.imread("images/junboundingrect.JPG"), Paper.LETTER)
    # areacalculator = AreaCalculator("GENEboundingrect",  cv2.imread("images/geneboundingrect.JPG"), Paper.LETTER)
    # areacalculator = AreaCalculator("TIRMIZIboundingrect",  cv2.imread("images/tirmiziboundingsquare.JPG"), Paper.LETTER)
